...
/**
 * cmd_line_loops - This function loops until the user enters "exit"
 ...
 * @env_head: A pointer to head of environ list.
 *
 * Return: 0 on success or 1 if _realloc fails.
 *
...
int cmd_line_loops(char *buffer, char *line, list_t **env_head)
{
	...
	while (1)
	{
		...
		if (line[0] != '\0')
		{
			...
			if (exit_shell(line_tok) && retval != -1)
		{
			...
			if (retval == -1)
			{
				/* not builtin, check if binary executable */
